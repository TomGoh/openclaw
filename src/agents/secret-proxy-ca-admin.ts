...
export type AddWhitelistViaCaParams = {
  caBaseUrl: string;
  adminToken: string;
  pattern: string;
  actor?: string;
...
/**
 * POST /admin/whitelist/add — add one endpoint prefix to TA whitelist.
 */
export async function addWhitelistViaCa(
  params: AddWhitelistViaCaParams,
  fetchFn: typeof fetch = fetch,
): Promise<{ ok: boolean; pattern: string; added?: boolean }> {
  const base = normalizeCaBase(params.caBaseUrl);
  const url = `${base}/admin/whitelist/add`;
  const res = await fetchFn(url, {
    method: "POST",
    headers: buildAdminHeaders({
      adminToken: params.adminToken,
  ...
  try {
    ...
    throw new Error(`CA add whitelist failed (${res.status}): ${text}`);
  }
  if (!res.ok) {
    const errMsg = typeof parsed.error === "string" ? parsed.error : text || `HTTP ${res.status}`;
    throw new Error(`CA add whitelist failed (${res.status}): ${errMsg}`);
  }
  if (!parsed.ok) {
    const errMsg = typeof parsed.error === "string" ? parsed.error : "add whitelist rejected";
    throw new Error(errMsg);
  }
  return {
    ok: true,
    pattern: typeof parsed.pattern === "string" ? parsed.pattern : params.pattern,
    added: parsed.added,
  };
}
